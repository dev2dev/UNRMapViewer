...
//  UNRMapViewer
//
//  Created by Andrew Dudney on 6/1/11.
...
namespace Matrix{
	...
	public:
		...
		inline void rotateX(float degrees){
			Matrix3D mat;
		...
		inline void rotateZ(float degrees){
			Matrix3D mat;
			float rad = DEGREES_TO_RADIANS(degrees);
		...
		inline void uniformScale(float s){
			Matrix3D mat;
			mat.scale(s, s, s);
			*this *= mat;
			//scale(s, s, s);
		...
		inline void orthographic(float left, float right, float bottom, float top, float near, float far){
			Matrix3D mat;
		...
		inline void perspective(float fov, float near, float far, float aspect){
			float size = near * tanf(DEGREES_TO_RADIANS(fov) / 2.0f);
			Matrix3D mat;
			mat.frustum(-size, size, -size / aspect, size / aspect, near, far);
			*this *= mat;
